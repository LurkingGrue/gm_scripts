...
if(loc.match(/\.org$/)){
dbS={row:[
	[/userscripts\.org/,[/scripts\/show\/6499$/,function(){if($(/Version\:\<\/b\>(.+?)\<br\/\>/,$('I:summary').innerHTML)!=sVer)alert('Please update to the latest version of NoDelay')}]]
...
dbS={row:[
 [/filebase/
// ,[/\/files[\/]/,function(){location.href=location.href.replace(/\/files[\/]/,'download')}]
 ,[/\./,function(){$rmA(['I:adlayerad','I:werbelayer-1']);$captcha('N:uid');$click('N:wait')}]
// 	 $captcha('N:fb_catcha');if(!$click('N:wait'))if(!$click('N:Download'))if(!$goCLOSE('A:href',$('X:Download','span').parentNode))$subCLOSE('N:waitform')}]
]
...
//,[/x7/,[/\./,function(){alert(8)}]//var fA=function(){alert(9);if($('I:requestTicket'))alert(0)};$wait(5,'X7',fA)}]//alert('/james/ticket/dl/'+$(/\.to\/(.+?)[\/]/))}]]
//AD's below
...
dbS={row:[
 [/zshare/
 ,[/download[\/]/,function(){var fA=function(){if(unsafeWindow.link_enc){var link='';for(i=0;i<unsafeWindow.link_enc.length;i++){link+=unsafeWindow.link_enc[i]};$goCLOSE(link);}};if(!$sub('N:form1'))$wait($getINT(/here\|(\d+)\|class/),'ZS',fA)}]
...
,[/filesend/,[/download\.php/,function(){unsafeWindow.time=0;var fA=function(){$clickCLOSE('C:buttdl');};$wait(1,'FS',fA)}]]
...
,[/hotfile/,[/dl/,function(){$captcha('I:recaptcha_response_field');try{unsafeWindow.starttimer();N=$getINT(/[wait|begin][\s|.+?](\d+) seconds/);if(N==0)N=$getINT(/wait[\s|.+?](\d+) minutes/,60);if(N>0)$wait(N+2,'HF','hide');else{$click('V:Download the file')}}catch(e){$goCLOSE('A:href','X:Click here to download')}}]]
,[/rapidshare/
 ,[/files/,function(){
 	...
 	if(!$wait($int(unsafeWindow.c),'RS',fASub))
...
,[/sendspace/
 ...
 ,[/file/,function(){if(N=$('N:download'))$click(N);else{if(!$goCLOSE('A:href','I:downlink'))$goCLOSE(/href="(.+?)" /,unsafeWindow.link_dec)}}]
]
...
,[/egoshare/,[/download\.php/,function(){if(data=$(/loadfilelink.decode\(\"(.+?)\"\); document.location/))$goCLOSE(unsafeWindow.loadfilelink.decode(data))}]]
...
,[/eufiles/,[/\.com\/.+\/file/,function(){unsafewindow.timeout='1'}]]
...
*/
 ...
 $back=function(){unsafeWindow.back()};
 $reload=function(){unsafeWindow.reload()};
 $close=function(){unsafeWindow.close()};
